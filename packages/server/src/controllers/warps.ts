...
export default class Warp {
    ...
    public warp(player: Player, id: number): void {
        ...
        if (!player.quests.isTutorialFinished())
            return player.notify(`Вы должны закончить учёбу перед началом искривления.`);

        if (player.inCombat())
            return player.notify(`Вы должны подождать 10 секунд после окончания боя, чтобы переместиться.`);

        // Prevent teleporting too often.
        if (!this.isCooldown(player))
            return player.notify(`Вы должны подождать еще ${this.getDuration(player)} перед искривлением`);

        let warp = this.getWarp(id);

        // No warp found.
        if (!warp) return log.warning(`Не удалось найти искривление с идентификатором ${id}.`);
    ...
    private teleport(player: Player, warp: ProcessedArea): void {
        ...
        player.notify(`Вы были искажены, чтобы ${Utils.formatName(warp.name)}!`);
    }
    ...
    private hasRequirement(player: Player, warp: ProcessedArea): boolean {
        ...
        if (warp.level && player.level < warp.level) {
            player.notify(`Вы должны быть уровня ${warp.level} чтобы переместится в ${warp.name}.`);
            return false;
        }
        ...
        if (warp.quest && !player.quests.get(warp.quest)?.isFinished()) {
            ...
            if (!quest?.isFinished()) {
                player.notify(
                    `Закончите квест  ${quest.name} чтобы переместиться в ${Utils.formatName(warp.name)}.`
                );
                return false;
            }
        }
        ...
        if (warp.achievement && !player.achievements.get(warp.achievement)?.isFinished()) {
            player.notify(`Перед использованием этого Искривления необходимо выполнить достижение.`);
            return false;
        }
