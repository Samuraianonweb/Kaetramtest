...
export default class Guilds {
    ...
    public create(
        ...
        if (player.guild) return player.notify('Вы уже состоите в гильдии.');

        // Prevent players from creating a guild if they haven't finished the tutorial.
        if (!player.quests.isTutorialFinished())
            return player.guildNotify('Перед созданием гильдии вы должны завершить обучение.');

        // Ensure the player has enough gold to create a guild.
        if (!player.inventory.hasItem('gold', 30_000))
            return player.guildNotify('Для создания гильдии вам необходимо иметь в инвентаре 30 000 золотых.');
        ...
        this.database.loader.loadGuild(identifier, (guild?: GuildData) => {
            ...
            if (guild) return player.guildNotify('Гильдия с таким названием уже существует.');
    ...
    public join(player: Player, identifier: string): void {
        ...
        this.database.loader.loadGuild(identifier, (guild?: GuildData) => {
            if (!guild)
                return log.general(
                    `Игрок ${player.username} пытался вступить в несуществующую гильдию.`
                );

            // Ensure the guild isn't full.
            if (guild.members.length >= Modules.Constants.MAX_GUILD_MEMBERS)
                return player.notify('Эта гильдия уже работает на пределе возможностей.');
    ...
    public leave(player: Player): void {
        ...
        if (!player.guild)
            return log.warning(`${player.username} пытались покинуть гильдию, в которой они не состоят.`);

        // Grab the guild from the database.
        this.database.loader.loadGuild(player.guild, (guild?: GuildData) => {
            if (!guild)
                return log.general(
                    `Игрок ${player.username} пытался покинуть несуществующую гильдию.`
                );
    ...
    public chat(player: Player, message: string): void {
        if (!player.guild)
            return log.warning(`${player.username} пытались общаться в гильдии, в которой они не состоят.`);

        this.database.loader.loadGuild(player.guild, (guild?: GuildData) => {
            if (!guild)
                return log.general(
                    `Игрок ${player.username} пытался написать в чат несуществующей гильдии.`
                );
