    ...
    private handlePlayerCommands(command: string, blocks: string[]): void {
        switch (command) {
            case 'players': {
                ...
                this.player.notify(
                    ` Здесь${singular ? 'is' : 'are'} в настоящее время ${population} ${
    ...
    private handleModeratorCommands(command: string, blocks: string[]): void {
        ...
        switch (command) {
            ...
            case 'ban': {
                ...
                if (!duration || !targetName)
                    return this.player.notify(
                        'Некорректная команда, ожидается /ban(mute) [продолжительность] [имя пользователя]'
                    );

                let user: Player = this.world.getPlayerByName(targetName);

                if (!user)
                    return this.player.notify(`Не удалось найти игрока с именем: ${targetName}.`);
                ...
                if (command === 'mute') {
                    ...
                    this.player.notify(`${user.username} был приглушен на ${duration} часов.`);
                } else if (command === 'ban') {
                    user.ban = timeFrame;
                    ...
                    this.player.notify(`${user.username} был забанен на ${duration} часов.`);
                }
            ...
            case 'unmute': {
                ...
                if (!uTargetName) return this.player.notify(`Игрок ${uTargetName} не найден`);

                uUser.mute = Date.now() - 3600;

                uUser.save();

                this.player.notify(`${uUser.username} был розмучен.`);
            ...
            case 'forcekick': {
                ...
                if (!username)
                    return this.player.notify(`Некорректная команда, ожидалось /kick имя пользователя`);

                let player = this.world.getPlayerByName(username);

                if (!player)
                    return this.player.notify(`Не удалось найти игрока с именем: ${username}`);
    ...
    private handleAdminCommands(command: string, blocks: string[]): void {
        ...
        switch (command) {
            case 'spawn': {
                ...
                if (!item.exists) return this.player.notify(`Элемента с ключом ${key} не существует.`);
            ...
            case 'take': {
                ...
                if (!index || !username)
                    return this.player.notify(
                        'Неверная команда, использование /take [index] [container=bank/inventory] [username]'
                    );

                let player = this.world.getPlayerByName(username);

                if (!player) return this.player.notify(`Игрок ${username} не найден.`);

                let containerType = container === 'inventory' ? player.inventory : player.bank,
                    slot = containerType.get(index);

                if (!slot.key)
                    return this.player.notify(`У игрока ${username} нет предмета в индексе ${index}.`);

                containerType.remove(index, slot.count);

                this.player.notify(`Взято ${slot.count}x ${slot.key} из ${username}.`);
            ...
            case 'takeitem': {
                ...
                if (!key || !username || (container !== 'inventory' && container !== 'bank'))
                    return this.player.notify(
                        'Неверная команда, использование /takeitem [key] [count] [container=bank/inventory] [username]'
                    );
                ...
                this.player.notify(`Взято ${count}x ${key} от ${username}.`);
            ...
            case 'copyinventory': {
                ...
                if (!username)
                    return this.player.notify('Неверная команда, использование /copybank [имя пользователя]');

                let player = this.world.getPlayerByName(username);

                if (!player) return this.player.notify(`Игрок ${username} не онлайн.`);
                ...
                this.player.notify(`Скопируйте ${command} из ${username} в свою ${command}.`);
            }
            ...
            case 'invincible': {
                if (this.player.status.has(Modules.Effects.Invincible)) {
                    ...
                    this.player.notify('Вы больше не непобедимы.');
                } else {
                    this.player.status.add(Modules.Effects.Invincible);

                    this.player.notify('Теперь вы непобедимы.');
                }
                return;
            }
            ...
            case 'mob': {
                ...
                if (!target) return this.player.notify('Мобы не указаны.');
            ...
            case 'allattack': {
                ...
                if (!target)
                    return this.player.notify(
                        `Неверная команда. Использование: /allattack [target_instance]`
                    );

                if (!region) return this.player.notify('Братан что.');
            ...
            case 'getregion': {
                this.player.notify(`Текущий регион: ${this.player.region}`);
                return;
            }
            ...
            case 'setlevel': {
                ...
                if (!username || !key || !x)
                    return this.player.notify(
                        'Ошибочная команда, ожидается /setlevel [навык] [уровень] [имя пользователя]'
                    );
                ...
                if (!skill) return this.player.notify('Неверный навык.');
            ...
            case 'ms': {
                ...
                if (!movementSpeed) {
                    this.player.notify('Скорость передвижения не указана.');
                    return;
                }
            ...
            case 'resetquest': {
                ...
                if (!key) return this.player.notify('Квест не указан.');
            ...
            case 'movenpc': {
                ...
                if (!instance)
                    return this.player.notify(`Неправильно сформированная команда, ожидалось /movenpc instance x y`);

                entity = this.entities.get(instance) as Character;

                if (!entity) return this.player.notify(`Сущность не найдена.`);
            ...
            case 'nvn': {
                ...
                if (!instance || !target)
                    return this.player.notify(`Некорректная команда, ожидаемая цель экземпляра /nvn`);

                entity = this.entities.get(instance) as Character;
                targetEntity = this.entities.get(target) as Character;

                if (!entity || !targetEntity)
                    return this.player.notify(`Не удалось найти указанные экземпляры сущностей.`);

                entity.combat.attack(targetEntity);

                this.player.notify(`${entity.name} нападает ${targetEntity.name}`);
            ...
            case 'kill': {
                ...
                if (!username)
                    return this.player.notify(
                        `Некорректная команда, ожидалось /kill username/instance`
                    );
            ...
            case 'finishquest': {
                ...
                if (!questKey)
                    return this.player.notify(`Некорректная команда, ожидалось /finishquest questKey`);

                quest = this.player.quests.get(questKey);

                if (quest) quest.setStage(9999);
                else this.player.notify(`Не удалось найти квест с ключом: ${questKey}`);
            ...
            case 'finishachievement': {
                ...
                if (!achievementKey)
                    return this.player.notify(
                        `Некорректная команда, ожидалось /finishachievement achievementKey`
                    );
            ...
            case 'roam': {
                ...
                if (!region) this.player.notify('Брат, что-то пошло не так.');

                this.player.notify(`Все мобы в регионе теперь будут бродить!`);
            ...
            case 'distance': {
                ...
                if (!x || !y)
                    return this.player.notify(`Некорректная команда, ожидалось /distance x y`);
            ...
            case 'nuke': {
                ...
                this.player.notify(
                    'Поздравляю, ты убил всех, ты доволен собой?'
                );
