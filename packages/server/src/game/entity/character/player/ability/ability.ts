...
export default class Ability {
    ...
    public activate(player: Player): boolean {
        ...
        if (!mana) {
            log.warning(`Способность ${this.key} без маны`);
            return false;
        }

        // Ensure active abilities have a cooldown and duration.
        if (!cooldown || !duration) {
            log.warning(`Способность ${this.key} не имеет времени действия или продолжительности.`);
            return false;
        }

        // Player doesn't have enough mana.
        if (player.mana.getMana() < mana) {
            player.notify('У вас недостаточно маны для использования этой способности.');
            return false;
        }
        ...
        if (this.isCooldown(cooldown)) {
            player.notify(
                `Вам нужно подождать ${Math.floor(
                    (cooldown - (Date.now() - this.lastActivated)) / 1000
                )} секунд чтобы использовать способность снова .`
            );
            return false;
        }
