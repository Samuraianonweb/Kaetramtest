...
export default class SecretCalling extends Ability {
    public constructor(level: number, quickSlot = -1) {
        super('secretcalling', level, quickSlot);
    }
    ...
    public override activate(player: Player): boolean {
        if (!player.hasTarget()) {
            player.notify(`Вы должны находиться в бою, чтобы использовать эту способность.`);
            return false;
        }
