...
export default class Friends {
    ...
    public add(player: Player | string): void {
        ...
        if (username.length > 32) return this.player.notify('Это имя пользователя слишком длинное.');

        if (username === this.player.username)
            return this.player.notify(
                `Слушай, я понимаю, тебе одиноко, но ты не можешь добавить себя в список друзей.`
            );

        // Ensure the player is not already on the list.
        if (this.hasFriend(username))
            return this.player.notify('Этот игрок уже есть в вашем списке друзей,мне кажется у вас деменция.');

        // Ensure the player exists.
        this.player.database.exists(username, (exists: boolean) => {
            if (!exists) return this.player.notify('Игрока с таким именем пользователя не существует.');
    ...
    public remove(player: Player | string): void {
        ...
        if (!this.hasFriend(username))
            return this.player.notify('Этого игрока нет в вашем списке друзей.');
