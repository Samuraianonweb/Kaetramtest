...
export default class Trade {
    ...
    public add(index: number, count = 1): void {
        ...
        if (offerIndex === -1)
            return this.player.notify(`Вы не можете больше добавлять товары в торговлю.`, '', 'TRADE');
        ...
        if (item.undroppable) return this.player.notify(`Вы не можете обменять этот предмет.`, '', 'TRADE');
    ...
    public accept(): void {
        ...
        this.acceptCallback?.('Вы приняли сделку.');
        this.getActiveTrade()?.acceptCallback?.('Другой игрок принял сделку.');
    }
    ...
    public request(target: Player): void {
        ...
        if (this.player.isCheater())
            return this.player.notify('Извините, но мошенники не допускаются к торговле.');

        if (target.isCheater())
            return this.player.notify('Этот игрок - мошенник, он может продать вам контрабанду!');
        ...
        target.notify(
            `${Utils.formatName(this.player.username)} обратился с просьбой о торговли с вами.`,
            'rgb(84, 224, 255)',
            'TRADE'
        );

        this.player.notify(
            `Вы обратились с просьбой о торговли с${Utils.formatName(target.username)}.`,
            '',
            'TRADE'
        );
    }
    ...
    public open(target: Player): void {
        log.debug(`Открытие торговли между ${this.player.username} и${target.username}.`);
    ...
    private notEnoughSpace(): void {
        ...
        this.activeTrade?.notify(
            `У другого игрока недостаточно места в инвентаре.`,
            '',
            'TRADE'
        );
        return this.player.notify(`У вас недостаточно места в инвентаре.`, '', 'TRADE');
    }
    ...
    private exchange(): void {
        ...
        if (flagged) {
            this.player.notify(`Пожалуйста, сообщите об ошибке, произошла ошибка.`, '', 'TRADE');
            log.warning(
                `Торговый обмен не состоялся для ${this.player.username} and ${this.activeTrade?.username}.`
            );
            return this.close();
        }
        ...
        if (totalItems === 0) {
            this.player.notify(`Йо, почему вы все торгуете ничем?`, '', 'TRADE');
            this.activeTrade?.notify(`Йо, почему вы ничем не торгуете?`, '', 'TRADE');
        } else {
            // Notify both that the trade is complete.
            this.player.notify(`Благодарим вас за использование торговой системы!`, '', 'TRADE');
            this.activeTrade?.notify(`Благодарим вас за использование торговой системы!`, '', 'TRADE');
    ...
    private produceTradeLog(): string {
        ...
        return `Игрок ${this.player.username} торговал${offeredItemsArry}этим ${receivedItemsArry} с игроком${this.activeTrade?.username}`;
    }
