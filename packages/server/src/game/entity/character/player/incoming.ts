...
export default class Incoming {
    ...
    private handleReady(data: ReadyPacket): void {
        ...
        this.world.discord.sendMessage(this.player.username, 'вошел в систему!');
    ...
    private handleNetwork(message: [Opcodes.Network]): void {
        ...
        switch (opcode) {
            case Opcodes.Network.Pong: {
                ...
                this.player.notify(`Задержка,пинг ${time - this.player.pingTime}ms`, 'red');
    ...
    private handleChat(message: [string]): void {
        ...
        if (this.player.isMuted()) return this.player.notify('В настоящее время вы отключены,и не можете писать сообщения.', 'crimson');
    ...
    private handleExamine(instance: string): void {
        ...
        if (!entity.description) return this.player.notify('Я понятия не имею, что это такое.');
