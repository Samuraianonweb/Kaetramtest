...
export default class Handler {
    ...
    private handleClose(): void {
        ...
        this.world.discord.sendMessage(this.player.username, 'вышел из системы, надеюсь ты отлично проведёшь время !');
    ...
    private handleAttack(): void {
        if (this.player.isMagic()) {
            ...
            if (!this.player.hasManaForAttack())
                return this.player.notify('У вас мало маны, ваши атаки будут слабее.');
        ...
        if (this.player.isArcher()) {
            if (!this.player.hasArrows())
                return this.player.notify('У вас нет стрел для стрельбы,стреляю пальцем!');
    ...
    private handleDoor(door: ProcessedDoor): void {
        ...
        if (this.player.level < door.level)
            return this.player.notify(`Вы должны быть уровня ${door.level}, чтобы войти в эту дверь.`);
        ...
        if (door.reqAchievement) {
            ...
            if (!achievement?.isFinished())
                return this.player.notify(
                    `Чтобы пройти через эту дверь, вам нужно выполнить достижение ${achievement?.name}.`
                );
        }
        ...
        if (door.reqQuest) {
            ...
            if (!quest?.isFinished())
                return this.player.notify(
                    `Чтобы пройти через эту дверь, вам нужно выполнить квест ${quest?.name}.`
                );
        }
        ...
        if (door.reqItem) {
            ...
            if (!this.player.inventory.hasItem(door.reqItem, count))
                return this.player.notify(
                    'У вас нет необходимого ключа, чтобы пройти через эту дверь.'
                );

            this.player.inventory.removeItem(door.reqItem, count);

            this.player.notify(`Ключ рассыпается в пыль, когда вы проходите через дверь.`);
        }
    ...
    private handlePoison(type = -1, exists = false): void {
        ...
        if (type === -1) this.player.notify('Яд выветрился.');
        else if (exists) this.player.notify(`Вы были отравлены!`);
