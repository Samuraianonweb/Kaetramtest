...
export default class Player extends Character {
    ...
    public respawn(): void {
        ...
        if (!this.dead) return log.warning(`Неверный запрос на респаун.`);
    ...
    public welcome(): void {
        if (this.isNew()) return this.notify(`С возвращением${config.name}!`);

        this.notify(`С возвращеним ${config.name}!`);

        let population = this.world.getPopulation();

        if (population > 1)
            this.notify(`В настоящее время в сети находится ${population} игроков.`, '', '', true);
    }
    ...
    private verifyCollision(x: number, y: number): boolean {
        ...
        if (isColliding) {
            ...
            this.notify(`Обнаружен ноклип в точках ${x}, ${y}. Пожалуйста, отправьте сообщение об ошибке.`);
    ...
    public handleContainerRemove(type: Modules.ContainerType, index: number, count: number): void {
        if (count < 1 || isNaN(count)) return this.notify('Вы ввели недопустимую сумму.');
        ...
        if (type === Modules.ContainerType.Inventory && this.map.isDoor(this.x, this.y))
            return this.notify('Вы не можете бросать предметы, стоя в дверях.');
    ...
    public handleUserAgent(userAgent: string, regionsLoaded = 0): void {
        ...
        log.debug(`Сброс пользовательского агента и регионов, загруженных для${this.username}.`);
    }
    ...
    public handleMovementStarted(x: number, y: number, speed: number, target: string): void {
        ...
        if (speed !== this.getMovementSpeed())
            this.incrementCheatScore(`${this.username} Получена неправильная скорость передвижения.`);
    ...
    public handleMovementStep(x: number, y: number, timestamp = Date.now()): void {
        ...
        if (this.verifyMovement(x, y, timestamp))
            this.incrementCheatScore(`Несоответствие скорости движения: ${Date.now() - this.lastStep}`);
    ...
    public handleMovementStop(x: number, y: number, target: string, orientation: number): void {
        ...
        if (!this.moving) this.incrementCheatScore('Не получили пакет о начале движения.');
        ...
        if (entity?.isItem()) {
            ...
            if (!entity.isOwner(this.username))
                return this.notify(
                    `Этот товар можно забрать только ${Utils.formatName(entity.owner)}.`
                );
    ...
    public updatePVP(pvp: boolean): void {
        ...
        if (this.pvp && !pvp) this.notify('Вы больше не находитесь в зоне PvP!');
        else this.notify('Вы вошли в зону PvP!');
    ...
    public updateMinigame(info?: Area): void {
        ...
        if (entering) {
            ...
            this.notify('Добро пожаловать в лобби TeamWar!');
    ...
    public setSnowPotion(): void {
        ...
        this.status.addWithTimeout(
            ...
            () => {
                this.notify('Ваш иммунитет к эффектам замораживания ослаб.');
            }
        );

        this.notify(
            `Теперь у вас есть иммунитет к эффектам замораживания на${
    ...
    public setFirePotion(): void {
        ...
        this.status.addWithTimeout(
            ...
            () => {
                this.notify('Ваш иммунитет к эффектам огня ослаб.');
            }
        );

        this.notify(
            `Теперь у вас есть иммунитет к эффектам огня на${
    ...
    public setPet(key: string): void {
        if (this.pet) return this.notify(`У вас уже есть домашнее животное!`);
    ...
    public sendPrivateMessage(playerName: string, message: string): void {
        ...
        if (!this.world.isOnline(playerName))
            return this.notify(`@aquamarine@${playerName}@crimson@ не в сети.`, 'crimson');
    ...
    public chat(message: string, global = false, withBubble = true, colour = ''): void {
        if (!this.canTalk) return this.notify('Вы не можете говорить в это время.', 'crimson');
