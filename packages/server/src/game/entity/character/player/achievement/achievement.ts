...
export default class Achievement {
    ...
    private getDiscoveredPopup(): PopupData {
        return {
            title: 'Достижение раскрыто',
            text: `${this.name} был обнаружен!`,
            colour: '#33cc33'
        };
    }
    ...
    private getFinishPopup(): PopupData {
        let text = `@green@Вы выполнили достижение@crimson@${this.getName()}@green@!`;

        if (this.rewardExperience > 0)
            text = `@green@ Вы получили @crimson@${this.rewardExperience} опыт@green@.`;
        else if (this.rewardAbility)
            text =
                this.rewardAbilityLevel === 1
                    ? `@green@ Вы разблокировали @crimson@${this.rewardAbility}@green@ достижение.`
                    : `@green@ Ваша @crimson@${this.rewardAbility} способность теперь на уровне ${this.rewardAbilityLevel}@green@.`;
        else if (this.secret)
            text = `@green@Вы выполнили секретное достижение @crimson@${this.getName()}@green@!`;

        return {
            title: 'Достижение выполнено!',
            text,
            colour: '#33cc33'
        };
    }
