    ...
    protected canAttack(target: Character): boolean {
        if (target.isMob()) {
            ...
            if (this.isPlayer() && !this.quests.canAttackInTutorial()) {
                this.notify('У вас нет причин нападать на этих милых существ.');
                return false;
            }
        ...
        if (target.isCheater()) {
            this.notify(`Этот игрок - читер, не стоит нападать на такого человека!`);
        ...
        if (Math.abs(this.level - target.level) > 30) {
            this.notify('Вы не можете атаковать кого-то более чем на 30 уровней выше или ниже вас.');
            return false;
        }
