...
export default class Item extends Entity {
    ...
    public canEquip(player: Player): boolean {
        ...
        if (this.skill) {
            ...
            if (skill)
                if (skill.level < requirement) {
                    ...
                    player.notify(
                        `Ваш${skill.name} уровень должен быть не ниже ${requirement} для одеть этого предмета.`
                    );
                    return false;
        ...
        if (player.level < requirement) {
            player.notify(`Ваш общий уровень должен быть не менее ${requirement} для оснащения этого предмета.`);
            return false;
        }
        ...
        if (this.achievement && !player.achievements.get(this.achievement)?.isFinished()) {
            player.notify(`Для использования этого предмета требуется секретное достижение.`);
            return false;
        }
        ...
        if (this.quest) {
            ...
            if (!quest.isFinished()) {
                player.notify(`Вы должны выполнить ${quest.name} чтобы использовать этот предмет.`);
                return false;
            }
        }
